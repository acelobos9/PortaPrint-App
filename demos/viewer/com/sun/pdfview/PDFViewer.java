...
public class PDFViewer extends JFrame
    ...
    public final static String TITLE = "Porta Print - CvSU";
    /** The current PDFFile */
    PDFFile curFile;
    ...
    /// FILE MENU
    Action openAction = new AbstractAction("Open...", getIcon("gfx/open.gif")) {
    ...
    protected void init() {
        ...
        //toolbar.setMaximumSize(new Dimension(1200,200));
        JButton jb;
        
        jb = new javax.swing.JButton(openAction);
        jb.setContentAreaFilled(true);
        jb.setOpaque(true);
        //jb.isBorderPainted();
        
        //jb.setBorder([SynthBorder]);
...
//        System.out.println("Going to page " + pagenum);
    ...
    public void doOpen() {
        try {
            ...
            if(!f.exists()){
                ...
                FileNameExtensionFilter filterA = new FileNameExtensionFilter("PDF Files","pdf");
            
            FileSystemView fsv = new DirectoryRestrictedFileSystemView(f);
            JFileChooser fc = new JFileChooser(fsv);
            disableButton(fc, "FileChooser.homeFolderIcon");
            //disableButton(fc, "FileChooser.upFolderIcon");
            disableButton(fc, "FileChooser.newFolderIcon");
            //JFileChooser fc = new JFileChooser(f);
            fc.setCurrentDirectory(prevDirChoice);
            fc.setFileFilter(filterA);
            fc.setMultiSelectionEnabled(false);
            /*
            fc.setFileView(new FileView(){
                @Override
                public Boolean isTraversable(File fl) {
                    return f.equals(fl);
               }
            });*/
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                try {
                     //;f   c.getSelectedFile()
                    //openFile(new URL("http://studentaffairs.psu.edu/career/pdf/CG/CG_resume.pdf"));
    ...
    class PrintThread extends Thread {
        ...
        public void run() {
            try { 
                System.out.println("Set Printer by script");
                ptPages.show(ptPjob);
                ptPjob.print();
    ...
    public void valueChanged(TreeSelectionEvent e) {
        ...
        void askSource(){
            Object[] options1 = { "Flash Drive", "Online Server",
                "back" };
            int result = JOptionPane.showOptionDialog(null, "Where do you want to get your PDF?", "Select Source",
            ...
            if (result == JOptionPane.YES_OPTION){
                ...
                // initiate online file getter
                 askRef(0);
            }
        }
    
    void askRef(int status) {

            JTextField inputRefcode = new JTextField();
            final JComponent[] input = new JComponent[] {
                    new JLabel("Document Reference Code"),
                    inputRefcode
            };
            Object[] options1 = { "Get File",
                "back" };
            
            if(status==2){
                    JOptionPane.showMessageDialog(null,"No document found with code "+inputRefcode.getText());
            }
            int result = JOptionPane.showOptionDialog(null, input, "Enter Reference Code",
                JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE,
                null, options1, null);
            if (result == JOptionPane.YES_OPTION){
                FileSelector fsel = new FileSelector();
                try {
                    String codeResponse = fsel.fetchFile(inputRefcode.getText());
                    if("FAILED".equals(codeResponse)){
                        askRef(2);
                    }else{
                        openFile(new URL(fsel.fetchFile(inputRefcode.getText())));
                    }
